...
# Градиентный бустинг - регрессия (метрики)
class MyBoostReg():
    ...
    def __init__(self, n_estimators=10, learning_rate=0.1, max_depth=5,
                 min_samples_split=2, max_leafs=20, bins=16, loss='MSE', metric=None):
    ...
    def fit(self, X, y, verbose=False):
        ...
        for k in np.arange(0, self.n_estimators):
            ...
            self.showMetricsValue(k, verbose, y.values, y_predict)
        ...
        if self.metric is None:
            ...
            lossVector = self.getVectorLoss(y.values, y_predict)
...
if __name__ == '__main__':
    ...
    myBoostReg = MyBoostReg(n_estimators=100, learning_rate=0.5, max_depth=5,
                 min_samples_split=2, max_leafs=40, bins=16, loss='MSE', metric='RMSE')
    #print(myBoostReg)
