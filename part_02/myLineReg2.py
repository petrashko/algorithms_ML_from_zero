...
class MyLineReg():
    ...
    def show_estimators(self, n, verbose, y_target, y_predict):
        if (verbose > 0) and (n == 0 or (n+1) == self.verboseInc):
            ...
            if (self.metric != None) and (n == 0 or (n+1) == self.verboseInc):
...
if __name__ == '__main__':
    ...
    myLineReg = MyLineReg(n_iter=50, learning_rate=0.1, metric='mae')
    # Обучить модель
    myLineReg.fit(X, y, 5)
    print()
