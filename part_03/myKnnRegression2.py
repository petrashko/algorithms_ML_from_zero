...
# Метод взвешенных K-ближайших соседей (регрессия)
class MyKNNReg():
    ...
    def predict(self, X_test):
        ...
        for dots in dotsForX:
            ...
            # В вычислениях используем целевые значения и ранги
            if self.weight == 'rank':
                # Вычислить весовой коэффициент каждой точки
                rankBack = (1 / rankList)
                denominator = np.sum(rankBack)
                dotsWeight = rankBack / denominator
                # Вычислить сумму целевых значений, с учетом весовых коэффициентов
                y_predict.append( (dotsWeight * valueList).sum() )
            # В вычислениях используем целевые значения и расстояния
                ...
                y_predict.append( (dotsWeight * valueList).sum() )
            # Иначе,
            # В вычислениях используем только целевые значения
            else:
...
if __name__ == '__main__':
    ...
    #myKnnReg = MyKNNReg(k=3, metric='euclidean', weight='uniform')
