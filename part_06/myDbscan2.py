...
# DBSCAN - кластеризация (обучение и предсказание)
class MyDBSCAN():
    ...
    def getNeighbourList(self, num):
        ...
        for row in np.arange(0, self.X_train.shape[0]):
            ...
            if self.euclideanDistance(obj, obj2) < self.distance:
...
if __name__ == '__main__':
    ...
    myDBSCAN = MyDBSCAN(eps=5, min_samples=4)
    ...
    '''
    dots = X_test.to_numpy()
    
    numOutliers = []
    for n, pred in enumerate(predictList):
        if pred == 0:
            numOutliers.append(n)
    
    outlierDots = dots[numOutliers]
    print(len(outlierDots))
    
    xCoord = [dot[0] for dot in outlierDots]
    yCoord = [dot[1] for dot in outlierDots]
    plt.scatter(xCoord, yCoord, c='b')
    
    from itertools import cycle
    
    for c, rows in zip(cycle('grcmykgrcmykgrcmykgrcmykgrcmykgrcmyk'), myDBSCAN.clusterObjectsDict.values()):
        clusterDots = dots[rows]
        xCoord = [dot[0] for dot in clusterDots]
        yCoord = [dot[1] for dot in clusterDots]
        plt.scatter(xCoord, yCoord, c=c)
