...
# Дерево решений - классификация (важность признаков)
...
class MyTreeClf:
    ...
    def calculateFeatureImpotance(self, feature, y_left, y_right):
        # Значения целевых переменных (метки класса 0 и 1)
    ...
    # Метод возвращает Gini Impurity для вектора 'y'
    #
    ...
    def getInformationGain(self, left, right):
        ...
        if (cntLeft == 0) or (cntRight == 0):
            ...
            if self.criterion == 'gini':
                # Вычислить Gini Impurity для целого вектора
                infTotal = self.getGini(np.hstack((left, right)))
                # Вычислить Gini Impurity отдельно для каждого из подвекторов
...
if __name__ == '__main__':
    ...
    #myTreeClf = MyTreeClf(max_depth=1, min_samples_split=1, max_leafs=2, criterion='gini', bins=8)
    #myTreeClf = MyTreeClf(max_depth=3, min_samples_split=2, max_leafs=5, criterion='gini', bins=None)
    #myTreeClf = MyTreeClf(max_depth=5, min_samples_split=200, max_leafs=10, criterion='entropy', bins=4)
    #myTreeClf = MyTreeClf(max_depth=4, min_samples_split=100, max_leafs=17, criterion='gini', bins=16)
    #myTreeClf = MyTreeClf(max_depth=10, min_samples_split=40, max_leafs=21, criterion='gini', bins=10)
    myTreeClf = MyTreeClf(max_depth=15, min_samples_split=20, max_leafs=30, criterion='gini', bins=6)
    #myTreeClf = MyTreeClf(max_depth=3, min_samples_split=2, max_leafs=1, criterion='gini', bins=10)
    
    # Обучение
    myTreeClf.fit(X_train, y_train)
    #print()
    myTreeClf.printTree(myTreeClf.root); print()
    print(myTreeClf.lastLeaf, myTreeClf.testSum); print()
